...
<div class="header">
  ...
  <nav class="nav">
    ...
    <a id="nav-analytics" data-page="analytics">Analytics</a>
...
<div id="page-analytics" class="page">
  <div style="margin-bottom:20px">
    <h2 style="font-size:18px;font-weight:600;margin-bottom:4px">Analytics</h2>
    <p class="meta" style="margin:0">Price history and change tracking across all products.</p>
  </div>

  <!-- Price History Chart -->
  <div class="card">
    <h3 style="font-size:14px;font-weight:600;margin-bottom:12px">Price history</h3>
    <select id="chart-product-select" style="width:100%;padding:9px 12px;border:1px solid #ddd;border-radius:7px;font-size:14px;background:#fafafa;margin-bottom:14px">
      <option value="">Select a product...</option>
    </select>
    <div style="position:relative;height:280px">
      <canvas id="price-chart"></canvas>
    </div>
    <p id="chart-no-data" style="display:none;text-align:center;color:#999;font-size:13px;margin-top:16px">No price changes detected yet</p>
    <div id="chart-stats" style="display:none;margin-top:16px;display:grid;grid-template-columns:repeat(5,1fr);gap:8px">
    </div>
  </div>

  <!-- Price Change Table -->
  <div class="card" style="margin-top:16px">
    <h3 style="font-size:14px;font-weight:600;margin-bottom:12px">Price changes</h3>
    <table>
      <thead><tr>
        <th>Product</th>
        <th style="text-align:right">Current</th>
        <th style="text-align:right">24h ago</th>
        <th style="text-align:right">Change</th>
        <th style="text-align:center">Days tracked</th>
      </tr></thead>
      <tbody id="price-change-tbody"></tbody>
    </table>
    <p id="price-change-empty" style="display:none;text-align:center;color:#999;font-size:13px;margin-top:12px">No price data yet.</p>
  </div>
</div>
...
async function loadAnalytics() {{
  ...
  var prods = await sbSelect('products', 'is_excluded=eq.false&order=last_seen.desc&limit=100');
  analyticsProducts = prods || [];

  // Populate dropdown
  var sel = document.getElementById('chart-product-select');
  var prevVal = sel.value;
  sel.innerHTML = '<option value="">Select a product...</option>';
  for (var i = 0; i < analyticsProducts.length; i++) {{
    var p = analyticsProducts[i];
    var opt = document.createElement('option');
    opt.value = p.id;
    opt.textContent = (p.title || '').substring(0, 70) + ' (' + (p.source || '') + ')';
    sel.appendChild(opt);
  }}
  if (prevVal) sel.value = prevVal;

  // Load price change table
  await loadPriceChangeTable();
}}

async function loadChartForProduct(productId) {{
  ...
  var history = await sbSelect('price_history', 'product_id=eq.' + productId + '&order=scraped_at.asc');
  ...
  var labels = history.map(function(h) {{
    var d = new Date(h.scraped_at);
    return d.toLocaleDateString() + ' ' + d.toLocaleTimeString([], {{hour:'2-digit',minute:'2-digit'}});
  }});
  var prices = history.map(function(h) {{ return h.price_usd; }});

  // Check if all prices same
  var allSame = prices.every(function(p) {{ return p === prices[0]; }});
  ...
  if (allSame && prices.length > 1) {{
    noData.style.display = 'block';
    noData.textContent = 'No price changes detected yet';
  }}
  ...
  var points = prices.length;
  var firstDate = new Date(history[0].scraped_at);
  var lastDate = new Date(history[history.length - 1].scraped_at);
  var daysTracked = Math.max(1, Math.round((lastDate - firstDate) / 86400000));

  statsEl.style.display = 'grid';
  var items = [
    ['Current', '$' + current.toFixed(2)],
    ['All-time low', '$' + low.toFixed(2)],
    ['All-time high', '$' + high.toFixed(2)],
    ['Data points', points],
    ['Days tracked', daysTracked]
  ];
  items.forEach(function(s) {{
    var d = document.createElement('div');
    d.style.cssText = 'text-align:center;padding:8px;background:#fafafa;border-radius:6px';
    d.innerHTML = '<div style="font-size:16px;font-weight:600">' + s[1] + '</div><div style="font-size:11px;color:#888;margin-top:2px">' + s[0] + '</div>';
    statsEl.appendChild(d);
  }});
}}

async function loadPriceChangeTable() {{
  var tbody = document.getElementById('price-change-tbody');
  var emptyMsg = document.getElementById('price-change-empty');
  tbody.innerHTML = '';
  emptyMsg.style.display = 'none';

  if (!analyticsProducts.length) {{
    emptyMsg.style.display = 'block';
    return;
  }}

  var rowCount = 0;
  for (var i = 0; i < Math.min(analyticsProducts.length, 50); i++) {{
    var p = analyticsProducts[i];
    var hist = await sbSelect('price_history', 'product_id=eq.' + p.id + '&order=scraped_at.desc&limit=50');
    if (!hist || !hist.length) continue;

    var current = hist[0].price_usd;
    var price24h = null;
    var cutoff = Date.now() - 86400000;
    for (var j = 0; j < hist.length; j++) {{
      if (new Date(hist[j].scraped_at).getTime() <= cutoff) {{
        price24h = hist[j].price_usd;
        break;
      }}
    }}
    if (price24h === null && hist.length > 1) price24h = hist[hist.length - 1].price_usd;
    if (price24h === null) price24h = current;

    var change = current - price24h;
    var changePct = price24h ? ((change / price24h) * 100) : 0;
    var firstDate = new Date(hist[hist.length - 1].scraped_at);
    var daysTracked = Math.max(1, Math.round((Date.now() - firstDate.getTime()) / 86400000));

    var changeColor = change < 0 ? '#27ae60' : change > 0 ? '#e74c3c' : '#888';
    var arrow = change < 0 ? '\u2193' : change > 0 ? '\u2191' : '';
    var changeStr = change === 0 ? '\u2014' :
      '<span style="color:' + changeColor + '">' + arrow + ' $' + Math.abs(change).toFixed(2) + ' (' + (changePct >= 0 ? '+' : '') + changePct.toFixed(1) + '%)</span>';

    var tr = document.createElement('tr');
    tr.innerHTML =
      '<td style="padding:8px;border-bottom:1px solid #eee;font-size:13px">' + esc((p.title || '').substring(0, 55)) + '<br><span style="font-size:11px;color:#aaa">' + esc(p.source || '') + '</span></td>' +
      '<td style="padding:8px;border-bottom:1px solid #eee;text-align:right;font-weight:500">$' + current.toFixed(2) + '</td>' +
      '<td style="padding:8px;border-bottom:1px solid #eee;text-align:right;color:#888">$' + price24h.toFixed(2) + '</td>' +
      '<td style="padding:8px;border-bottom:1px solid #eee;text-align:right;font-size:13px">' + changeStr + '</td>' +
      '<td style="padding:8px;border-bottom:1px solid #eee;text-align:center;color:#aaa;font-size:12px">' + daysTracked + 'd</td>';
    tbody.appendChild(tr);
    rowCount++;
  }}

  if (!rowCount) emptyMsg.style.display = 'block';
}}
...
document.getElementById('chart-product-select').addEventListener('change', function() {{
  loadChartForProduct(this.value);
}});
